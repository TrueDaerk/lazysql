...
func (m Model) tableFKKey() fkKey {
	return fkKey{conn: m.active, database: m.grid.data.database, table: m.grid.data.table}
}

// namespaceFKKey identifies the browsed namespace for the reverse scan.
func (m Model) namespaceFKKey() fkKey {
	return fkKey{conn: m.active, database: m.grid.data.database}
}
...
func (m Model) tableFKs() ([]db.ForeignKey, bool) {
	fks, ok := m.grid.fkCache[m.tableFKKey()]
	return fks, ok
}
...
func (m *Model) ensureFKs() tea.Cmd {
	...
	k := m.tableFKKey()
	if _, ok := m.grid.fkCache[k]; ok {
		return nil
	}
	if m.grid.fkLoading[k] {
		return nil
	}
	if m.grid.fkLoading == nil {
...
func (m *Model) ensureNamespaceFKs() tea.Cmd {
	...
	return tea.Batch(
		logCmd("-- scan foreign keys of %d tables in %s",
			len(tables), displayDatabase(m.grid.data.database)),
...
func (m *Model) showIncomingRefs() tea.Cmd {
	...
	refs, ok := m.grid.refsCache[m.namespaceFKKey()]
	if !ok {
		m.grid.fkAfter = actIncomingRefs
		return m.ensureNamespaceFKs()
	}
	incoming := incomingFor(refs, m.grid.data.table)
	if len(incoming) == 0 {
		return logCmd("-- no table in %s references %s",
			displayDatabase(m.grid.data.database), m.grid.data.table)
	}
	...
	entries = append(entries, menuEntry{key: "esc", label: "cancel"})
	m.modal = &menuModal{title: "Referenced by — " + m.grid.data.table, entries: entries}
	return nil
}
...
func (m *Model) jumpToReferencing(ref namespaceFK) tea.Cmd {
	...
	return m.jumpTo(m.grid.data.database, ref.table, f)
}
...
func (m Model) cursorColumnName() (string, bool) {
	...
	return m.grid.data.cols[m.grid.data.col].Name, true
}
...
func (m Model) cursorRowValues(cols []string) ([]any, error) {
	if m.grid.data.row < 0 || m.grid.data.row >= len(m.grid.data.rows) {
		return nil, fmt.Errorf("no row under the cursor")
	}
	row := m.grid.data.rows[m.grid.data.row]
